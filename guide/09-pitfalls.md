...
### 52. 白天rest不恢复San导致资源充足也全灭 🆕
**问题**：NPC白天选择rest回家，`_restCooldownTimer`只恢复体力不恢复San。而且rest期间San自然衰减和恶性循环仍在持续。导致白天休息不仅不回精神，反而越休息精神越差。配合rest拦截条件太严格（San<35才放行），NPC在San 35~50区间被赶去干活→工作消耗San→恶性循环→全员精神崩溃。

**根因分析**：
1. `isSleeping=true`才恢复San（+0.10/dt），白天rest时isSleeping不是true
2. 清醒时自然衰减（-0.02/dt）不区分是否在休息
3. 恶性循环（-0.03/dt, San<30时）不区分是否在休息
4. rest拦截太严格：San<35才算"需要休息"

**解决方案**：
- 白天rest缓冲期增加San恢复（+0.06/dt）
- rest缓冲期跳过自然衰减和恶性循环
- rest拦截阈值放宽：San<50允许休息

**⚠️ 通用原则：任何"恢复"类行为（rest/eat/therapy）进行中，必须暂停相应属性的衰减机制，否则恢复行为形同虚设。**

### 53. _updateSleepState与_updateSchedule同帧竞争导致NPC"弹弹乐"死循环 🆕🔴
**这是v4.17~v4.18修复的最严重bug**。`_updateSleepState`在主循环中先于`_updateSchedule`执行。深夜时段：
1. `_updateSleepState`检测到"该睡觉但NPC不在家" → 把NPC从非宿舍室内（如medical/workshop）`_teleportTo('village', ...)` 踢出
2. `_updateSchedule` P0/P1检测到"NPC不在目标位置" → 导航NPC回室内
3. 下一帧又被踢出 → **每帧循环一次，同一秒内几十次进出门**

**影响范围**：陆辰336次、清璇194次、李婶108次重复进门。涉及6条触发路径：P0-3 health_critical、P0-4 stamina_critical、P0-5 medical_urgent、exhausted状态覆盖、sick状态覆盖、mental状态覆盖。

**修复方案（6处）**：
1. `_updateSleepState`：有`_priorityOverride`或`_taskOverride`时不从非宿舍室内踢出
2. `_updateSleepState`：有P0/P1覆盖时整个"该睡觉但不在家"导航块跳过
3. P0-4 `stamina_critical`：室内不踢出，暂停任务原地休息
4. P0-3 `health_critical`：已在宿舍跳过 + 室内原地休息
5. `exhausted`/`sick`/`mental` 状态覆盖：在室内/在medical时不触发
6. `_enterIndoor` 防抖：已在目标场景直接return

**⚠️ 通用原则：update循环中先后执行的子系统如果修改同一个NPC的状态（场景/位置/覆盖标志），必须互相感知对方的意图。_updateSleepState在踢出NPC前必须检查是否有P0/P1在控制该NPC。新增任何"把NPC从室内踢出"的逻辑，都要先问：有没有更高优先级的系统正在让NPC留在室内？**

### 54. NPC在室内场景时，低优先级的"回家/回宿舍"逻辑不应踢出NPC 🆕
NPC已在室内场景（如medical、workshop）执行任务时，`exhausted`（回宿舍休息）、`health_critical`（回宿舍）等低优先级行为不应该把NPC从当前室内踢出到village再走回宿舍。正确做法是**在当前室内原地休息**（暂停任务、触发rest恢复计时器），等恢复到阈值后再恢复任务。这避免了NPC在建筑间反复跑的低效行为，也消除了弹弹乐死循环的触发条件。**原则：室内NPC的紧急恢复应该就地解决，不要为了"回家"而把NPC踢出当前室内。只有在village（户外）的NPC才需要导航回宿舍。**
