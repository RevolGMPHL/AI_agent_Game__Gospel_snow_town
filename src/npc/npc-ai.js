...
(function() {
    ...
    proto._canChatWith = function(other) {
        ...
        // 【优化v4.17】基于 getResourceTension() 控制聊天——但精神差的NPC豁免（聊天是恢复San的重要途径）
        ...
        if (game && game.resourceSystem) {
            ...
            // 【改进】精神差(San<40)的NPC豁免聊天禁令——聊天是恢复精神的重要方式
            const needsMentalRecovery = this.sanity < 40 || (other && other.sanity < 40);
            if (needsMentalRecovery) {
                // 精神差时允许聊天恢复，不受资源紧张度限制
                return true;
            }
            if (tension >= 0.3) {
                // 中高紧张度：禁止聊天（精神好的NPC应该工作）
    ...
    proto._fallbackToRoleDefaultAction = function(game) {
        ...
        const ROLE_DEFAULT_ACTIONS = {
            ...
            'old_qian':     { target: 'dorm_a_door',    desc: '去宿舍A安抚大家',     stateDesc: '安抚' },
...
game.addEvent(`🤝 ${this.name} 邀请 ${companion.name} 一起去${GST.NPC.ACTION_TARGETS[action.target] || action.target}`);
    ...
    GST.NPC.ACTION_TARGETS = {
        ...
        'furnace_plaza':   '暖炉广场（户外广场，安抚/集会→恢复San值，⚠️不提供室内供暖）',
...
2. 上方有工作安排表供你参考。当资源紧张时你应该积极执行安排的任务，但当物资充足时你完全可以自主决定做什么。
3. 你可能有被分配的生存任务（见「你的任务」），这是参考信息。根据当前态势（见上方📊当前态势）自行判断是否需要执行。
3b. 🗳️【投票决策】如果刚开过紧急决策会议（见下方「投票决策分工」），你必须按投票结果行动！这是全体成员投票通过的方案，优先级仅次于生存紧急需求。
4. 🚨如果体力<30/健康<30/体温<35°C，必须立即回室内建筑休息（宿舍/医疗站/炊事房/工坊等）！priority=urgent！⚠️暖炉广场是户外的，不能取暖！
...
9. 你是一个有自主意志的角色。日程和任务只是参考建议，你根据当前局势（物资、身心状态、天气）和你的判断来行动。物资充足时，你完全可以选择休息恢复精神、和朋友聊天、探索废墟、或者在室内待着。⚠️暖炉广场是露天的，不提供供暖保护，取暖必须进室内建筑！
...
${isLateNight ? '🌙 现在是深夜了，除非有紧急事务，否则你应该回家休息（选rest）。' : `☀️ 白天时间，你可以自主决定做什么——采集资源、休息恢复、找人聊天、探索废墟……一切由你决定。${this.sanity < 50 ? '\n💡 精神状态不佳时，回家休息(rest)或找人聊天(go_to到人多的地方)都能恢复精神。' : ''}`}
【最近想法】${lastThought}
【最近记忆】
${recentMemories || '（暂无）'}
...
if (!action.target || !GST.NPC.ACTION_TARGETS[action.target]) {
                ...
                // 【修复v4.17】白天(6:00~21:00)拦截rest决策，除非确实需要休息
                // 【改进v4.7】放宽休息条件：San<50也算需要休息，不再把精神状态不佳的NPC赶去干活
                if (!isLateNight && action.type === 'rest') {
                    const reallyNeedRest = this.stamina < 25 || this.isSick || this.health < 25 || this.sanity < 50;
                    if (!reallyNeedRest) {
                        console.log(`[行动决策] ${this.name} 白天想rest但身心状况良好，跳过让日程接管`);
                        return; // 不执行，让日程系统接管
                    }
                }
                ...
                // 【拦截2】暴风雪/极寒暴风雪期间强制户外NPC回室内
                // 【修复v4.16】原来只检查'暴风雪'，不匹配'极寒暴风雪'，导致第4天拦截失效
                const weatherNow = game.weatherSystem && game.weatherSystem.currentWeather;
                const isBlizzard = weatherNow && (weatherNow.includes('暴风雪') || weatherNow.includes('极寒'));
                if (isBlizzard && this.currentScene === 'village') {
                ...
                if (this.bodyTemp < 33) {
                    if (action.type !== 'rest' && action.type !== 'go_to') {
                        ...
                        action.type = 'rest';
                        action.target = 'dorm_a_door';
                        action.reason = '严重失温，必须立即回室内取暖！';
                        action.priority = 'urgent';
